...
with open(resultdir+"result.txt","a") as f:
	f.truncate(0)
for i in [5,6,7,8]:
	test("AdaptiveCruiseController/safe/", "ACC"+str(i), "ACC"+str(i), 5, opt_comp)
for i in [5,6,7,8]:
	test("AdaptiveCruiseController/unsafe/", "ACC"+str(i), "ACC"+str(i), 5, opt_comp)
for i in [2,3,4,5]:
	test("DistributedController/", "DISC"+str(i), "DISC"+str(i), 10, opt_comp)
for i in [4,5,6]:
	test("Fischer/", "FISCS0"+str(i)+"-UBD0"+str(i), "F"+str(i), 6, opt_comp)
for i in [4,5,6]:
	test("Fischer/", "FISCU0"+str(i)+"-UBD0"+str(i), "F"+str(i), 6, opt_comp)
for i in [2,3,4]:
	test("Navigation/", "NAV_3_"+str(i), "NAV_3_"+str(i), 30, opt_single)
for i in [5,7,9]:
	test("TTEthernet/", "TTE"+str(i), "TTE"+str(i), 4, opt_comp)
for i in [1,2,3,4]:
	test("DutchRailwayNetwork/", "DRN_new", "BDR0"+str(i), 5, opt_comp)
